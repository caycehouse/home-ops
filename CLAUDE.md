...
- Each app's `ks.yaml` is a Flux `Kustomization` (not a kustomize one). It declares `dependsOn` (most stateful apps depend on `openebs` in `openebs-system`), `targetNamespace`, optional `components`, and `postBuild.substitute` (commonly `APP: <name>`, consumed by shared components).
