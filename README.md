...
- **Storage & Data Protection**: [OpenEBS](https://github.com/openebs/openebs) ZFS LocalPV provides local persistent volumes, with [kopiur](https://github.com/home-operations/kopiur) handling backups and restores. OpenEBS hostpath is retained as a non-default class for explicitly transient scratch/cache needs only. ZFS pool setup is documented in the [OpenEBS ZFS runbook](./docs/zfs-openebs.md). [spegel](https://github.com/spegel-org/spegel) improves reliability by running a stateless, cluster-local OCI image mirror.
